...
class T2L2(LightningModule):
    ...
    def __init__(
        ...
        # self.f1 = MultiLabelF1AtK(k=5).to('cuda')
    ...
    #         mashup_repeated = mashup_map.repeat(len(sample_idx), 1)  # [num_samples, dim]
    #         api_embeds = self.api_embed[sample_idx]  # [num_samples, dim]
            
    #         pred = torch.sum(mashup_repeated * api_embeds, dim=1)  # [num_samples]
    #         pred = pred.to('cuda')
    ...
    def training_step(self, batch: Any, batch_idx: int) -> STEP_OUTPUT:
        ...
        for x_item, y_item in zip(x, y):
            positive_idx, negative_idx, sample_idx, target = select_negative_samples(
                y_item, self.negative_samples_ratio)
            mashup_map = self.vector_space_map(x_item).unsqueeze(0)
            
            pred = torch.mm(mashup_map, self.api_embed[sample_idx].t())  # [1, num_apis]
    ...
    def validation_step(self, batch: Any, batch_idx: int) -> Optional[STEP_OUTPUT]:
        ...
        preds = torch.mm(mashups, self.api_embed.t())
