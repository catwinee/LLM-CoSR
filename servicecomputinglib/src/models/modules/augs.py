...
def NodeDrop(data, aug_ratio):
    ...
    drop_idx = torch.ones(x.shape[0]).to(torch.bool)
...
def EdgePerturb(data, aug_ratio):
    ...
    permute_num = max(1, int(edge_num * aug_ratio))

    edge_index = data.edge_index

    unif = torch.ones(2, node_num)
    add_edge_idx = unif.multinomial(permute_num, replacement=True)

    unif = torch.ones(edge_num)
    if edge_num - permute_num <= 0:
        keep_edge_idx = unif.multinomial(1, replacement=True)
    else:
...
def Subgraph(data, aug_ratio):
    ...
    sub_num = max(1, int(data.num_nodes * aug_ratio))
    ...
    if diff is None:
        diff_keep_num = 0
        diff_keep_idx = torch.randperm(0)[:0]
    else:
        diff_keep_num = min(sub_num - last_idx.shape[0], diff.shape[0])
        diff_keep_idx = torch.randperm(diff.shape[0])[:diff_keep_num]
...
def AttrMask(data, aug_ratio):
    ...
    mask_num = max(1, int(data.num_nodes * aug_ratio))
